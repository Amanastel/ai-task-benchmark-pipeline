...
def format_target_spec_trace(
    scope, root_error, width=TRACE_WIDTH, depth=0, prev_target=_MISSING, last_branch=True
):
    """
...
class UnregisteredTarget(GlomError):
    ...
    def get_message(self):
        ...
        msg = "target type %r not registered for '%s', expected one of registered types: %s" % (
            self.target_type.__name__,
            self.op,
            reg_types_str,
        )
...
class Path:
        ...
        for part in path_parts[offset:]:
            ...
            if isinstance(part, TType):
                ...
                if sub_parts[0] is not T:
                    raise ValueError('path segment must be path from T, not %r' % sub_parts[0])
                i = 1
    ...
    def from_text(cls, text):
        ...
        def create():
            ...
            if PATH_STAR:
                segs = [
                    _T_STAR if seg == '*' else _T_STARSTAR if seg == '**' else seg for seg in segs
                ]
...
def _format_path(t_path):
    ...
    if path_parts or not cur_t_path:
        return 'Path(%s)' % ', '.join(
            [_format_t(part) if type(part) is list else repr(part) for part in path_parts]
        )
    ...
    *func*.
    ...
    def __init__(self, func=None, args=None, kwargs=None):
        ...
        if not (callable(func) or isinstance(func, (Spec, TType))):
            raise TypeError('expected func to be a callable or T expression, not: %r' % (func,))
        if args is None:
            args = ()
        if kwargs is None:
            kwargs = {}
...
class Invoke:
    ...
    def __init__(self, func):
        if not callable(func) and not _is_spec(func, strict=True):
            raise TypeError('expected func to be a callable or Spec instance, not: %r' % (func,))
        self.func = func
        self._args = ()
    ...
    def __repr__(self):
        ...
        for i in range(len(self._args) // 3):
            ...
            fname = fname_map[op]
            if op in ('C', 'S'):
                kwargs = [(k, v) for k, v in _kwargs.items() if self._cur_kwargs[k] is _kwargs]
            else:
                kwargs = {}
                if args:
                    kwargs['args'] = args
    ...
    def glomit(self, target, scope):
        ...
        for i in range(len(self._args) // 3):
            ...
            if op == 'C':
                ...
                all_kwargs.update(
                    {k: recurse(v) for k, v in kwargs.items() if self._cur_kwargs[k] is kwargs}
                )
            elif op == '*':
                if args is not None:
    ...
    def __getattr__(self, name):
        if name.startswith('__'):
            raise AttributeError(
                ...
                f' To access the "{name}" attribute, use'
                f' T.__("{name[2:]}")'
            )
...
class Let:
    ...
    def glomit(self, target, scope):
        scope.update({k: scope[glom](target, v, scope) for k, v in self._binding.items()})
        return target
