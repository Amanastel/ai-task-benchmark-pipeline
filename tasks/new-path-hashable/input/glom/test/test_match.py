...
from glom import STOP, A, Coalesce, Fill, Ref, S, Switch, T, Val, glom
from glom.core import SKIP, Auto, Ref
...
def test_cruddy_json():
    ...
    # embed a build
    squished_json = Match(
        {'smooshed_json': Auto((json.loads, Match({'sub': Auto((json.loads, M == 1))})))}
    )
...
def test_pattern_matching():
    pattern_matcher = Or(
        And(Match(1), Val('one')), And(Match(2), Val('two')), And(Match(float), Val('float'))
    )
    ...
    factorial = (
        ...
        Ref(
            'fact',
            (lambda t: t - 1, (M == 0) & Fill(1) | (S(r=Ref('fact')), S, lambda s: s['r'] * s[T])),
        ),
    )
