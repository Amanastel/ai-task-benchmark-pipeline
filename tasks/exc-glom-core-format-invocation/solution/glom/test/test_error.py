...
from glom import Coalesce, GlomError, Or, Path, PathAccessError, S, Switch, T, glom
from glom.core import LAST_CHILD_SCOPE, ROOT, bbrepr, format_oneline_trace, format_target_spec_trace
from glom.matching import M, Match, MatchError, TypeMatchError
...
fb = FunctionBuilder(name='_raise_exc', body='raise Exception("unique message")', args=['t'])
...
"""
    ...
    actual = _make_stack(
        Match(Switch([(1, 1), ('a', 'a'), ([None], Switch([(1, 1), ('a', 'a'), ([None], T.a)]))]))
    )
    expected = """\
Traceback (most recent call last):
  File "test_error.py", line ___, in _make_stack
...
ERROR_CLASSES = (ValueError, NameError, AttributeError, ZeroDivisionError, SyntaxError, ImportError)
...
def test_all_public_errors():
    ...
    err_types = sorted([t for t in err_types if t is not glom.GlomError], key=lambda t: t.__name__)
