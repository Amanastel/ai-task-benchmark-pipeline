...
class Assign:
    ...
    def glomit(self, target, scope):
        ...
        _apply = lambda dest: _assign_op(dest=dest, op=op, arg=arg, val=val, path=path, scope=scope)
...
_BUILTIN_BASE_TYPES = [
    ...
    if not issubclass(v, tuple([t for t in _ALL_BUILTIN_TYPES if t not in (v, type, object)]))
]
