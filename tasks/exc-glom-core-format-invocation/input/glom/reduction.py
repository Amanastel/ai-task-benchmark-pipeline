...
from .core import MODE, GlomError, T, UnregisteredTarget, bbrepr, format_invocation, glom
from .grouping import ACC_TREE, CUR_AGG, GROUP, target_iter
...
class Fold:
    ...
    def __init__(self, subspec, init, op=operator.iadd):
        ...
        if not callable(op):
            raise TypeError(
                'expected callable for %s op param, not: %r' % (self.__class__.__name__, op)
            )
        if not callable(init):
            raise TypeError(
                'expected callable for %s init param, not: %r' % (self.__class__.__name__, init)
            )
