...
from glom.core import ROOT, UP, bbformat, bbrepr
...
def test_initial_integration():
    ...
    print('in: ', val)
    print('got:', ret)
    expected = {'a': 'c', 'name': 'good_name', 'e': ['f'], 'i': [{'j': 'k'}], 'n': 'O', 'p': 'zzz'}
    print('exp:', expected)
...
def test_stop():
    ...
    target = ['a', ' b', ' c ', '   ', '  done']
    assert glom(target, [(lambda x: x.strip(), lambda x: x if x else STOP, lambda x: x[0])]) == [
        'a',
        'b',
        'c',
        '',
...
def test_spec_and_recursion():
    ...
    assert glom(['a', 'b', 'c'], Call(list, args=(Spec(Call(reversed, args=(Spec(T),))),))) == [
        'c',
        'b',
        'a',
    ]
    assert glom(['cat', {'cat': 1}], T[1][T[0]]) == 1
    assert glom([['ab', 'cd', 'ef'], ''.join], Call(T[1], args=(Auto((T[0], [T[1:]])),))) == 'bdf'
...
def test_beyond_access():
    ...
    # 3
    target = {
        'system': {'planets': [{'name': 'earth', 'moons': 1}, {'name': 'jupiter', 'moons': 69}]}
    }
