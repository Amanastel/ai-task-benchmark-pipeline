...
from boltons.iterutils import chunked_iter, first, split_iter, unique_iter, windowed_iter
...
class Iter:
    ...
    def _add_op(self, opname, args, callback):
        return type(self)(
            subspec=self.subspec, _iter_stack=[(opname, args, callback)] + self._iter_stack
        )
    ...
    def map(self, subspec):
        ...
        *subspec* to each element of the iterable.
        ...
        return self._add_op(
            'map',
            (subspec,),
            lambda iterable, scope: imap(lambda t: scope[glom](t, subspec, scope), iterable),
        )
    ...
    def windowed(self, size):
        ...
        [(0, 1), (1, 2), (2, 3)]
        """
        return self._add_op('windowed', (size,), lambda it, scope: windowed_iter(it, size))
    ...
    def split(self, sep=None, maxsplit=None):
        ...
        *sep* is ``None``, ``split()`` **groups** separators. If empty lists
        ...
        """
        return self._add_op(
            'split', (sep, maxsplit), lambda it, scope: split_iter(it, sep=sep, maxsplit=maxsplit)
        )
...
class First:
    ...
    # default}))
    __slots__ = ('_default', '_first', '_spec')
