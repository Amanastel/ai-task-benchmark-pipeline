...
class TypeMatchError(MatchError, TypeError):
    ...
    def __init__(self, actual, expected):
        super().__init__("expected type {0.__name__}, not {1.__name__}", expected, actual)
...
def _handle_dict(target, spec, scope):
    ...
    if required:
        raise MatchError(
            "target missing expected keys: {0}", ', '.join([bbrepr(r) for r in required])
        )
    return result
...
def _glom_match(target, spec, scope):
    if isinstance(spec, type):
        ...
        for item in target:
            for child in spec:
                ...
                if target and not spec:
                    raise MatchError("{0!r} does not match empty {1}", target, type(spec).__name__)
        ...
        raise MatchError("{0}({1!r}) did not validate (non truthy return)", spec.__name__, target)
    elif target != spec:
        raise MatchError("{0!r} does not match {1!r}", target, spec)
    return target
    ...
    def __init__(self, cases, default=_MISSING):
        ...
        if type(cases) is not list:
            raise TypeError(
                ...
                f" or [(keyspec, valspec)] not: {type(cases)}"
            )
        self.cases = cases
        ...
        if not cases:
            raise ValueError(
                'expected at least one case in %s, got: %r' % (self.__class__.__name__, self.cases)
            )
        return
    ...
    def __init__(self, spec=T, **kwargs):
        ...
        def _get_arg_val(name, cond, func, val, can_be_empty=True):
            ...
            if not is_iterable(val):
                ...
                raise ValueError(
                    'expected %r argument to contain at least one value, not: %r' % (name, val)
                )
            for v in val:
                if not func(v):
                    raise ValueError('expected %r argument to be %s, not: %r' % (name, cond, v))
            return val
        ...
        self.types = _get_arg_val('type', 'a type', lambda x: isinstance(x, type), type_arg, False)

        if equal_to is not _MISSING:
            self.vals = (equal_to,)
            if one_of is not _MISSING:
                raise TypeError(
                    'expected "one_of" argument to be unset when "equal_to" argument is passed'
                )
        elif one_of is not _MISSING:
            if not is_iterable(one_of):
                raise ValueError('expected "one_of" argument to be iterable , not: %r' % one_of)
            if not one_of:
                raise ValueError(
                    'expected "one_of" to contain at least one value, not: %r' % (one_of,)
                )
            self.vals = one_of
        else:
            self.vals = ()
