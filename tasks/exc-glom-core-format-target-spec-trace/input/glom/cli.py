...
import glom
from glom import GlomError, Inspect, Path
...
def get_command():
    ...
    cmd.add(
        '--target-format', str, missing='json', doc='format of the source data (json or python)'
    )
...
def mw_handle_target(target_text, target_format):
    ...
    try:
        ...
        raise UsageError('could not load target data, got: %s: %s' % (e.__class__.__name__, e))
