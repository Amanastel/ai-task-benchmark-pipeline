...
def test_cli_spec_argv_target_stdin_basic(cc):
    ...
    res = cc.run(['glom', '--indent', '0', '--target-file', '-', BASIC_SPEC], input=BASIC_TARGET)
    assert res.stdout == BASIC_OUT
...
def test_usage_errors(cc, basic_spec_path, basic_target_path):
    ...
    res = cc.fail_1(
        ['glom', '--spec-file', basic_spec_path + 'abra', '--target-file', basic_target_path]
    )
    assert 'could not read spec file' in res.stdout  # TODO: stderr

    res = cc.fail_1(
        ['glom', '--spec-file', basic_spec_path, '--target-file', basic_target_path + 'abra']
    )
